...
def bind_model(trainer: Trainer):
    ...
    def infer(test_data : DataFrame):
        test_data = trainer.preprocessor.preprocess_test_data(test_data).batch(1) # 1
...
def main():
    ...
    model = LSTM_Model(optimizer = tf.keras.optimizers.Adam(learning_rate=0.01), 
                        loss = tf.losses.MeanSquaredError(), 
                        metrics = [tf.metrics.MeanAbsoluteError(), tf.metrics.RootMeanSquaredError()], 
                        epochs = 20, batch_size = 32) # 5, shuffle=False
